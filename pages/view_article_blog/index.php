...
$this->post = $this->sql->sendQuery("SELECT * FROM ".__PREFIX__."blog WHERE id = ".$this->id);
	
while($this->article = mysql_fetch_array($this->post)) {
print "\n\t<div id=\"article_top\">"
    			
    	    	. "\n<b> <a href=\"?page=view_article_blog&id=".$this->article['id']."\">".$this->article['title']."</a></b>"
    			. "\n<br /><i>Scrito da:<b>".$this->article['author']."</b> il ".$this->article['post_date']."</i>"
    			. "\n\t</div>"
    			. "<div id=\"article\">"
...
$this->comments = $this->sql->sendQuery("SELECT * FROM ".__PREFIX__."comments_blog WHERE blog_id = ".$this->id." ORDER BY id");
...
}elseif(mysql_num_rows($this->comments) > 0) {	
    while($row = mysql_fetch_array($this->comments)) {
        print "<div id=\"commento\">"
        	. "\n<i>Comento nr. ".$count." scrito il ".$row['data']."<br /><b>Scritto da:</b>".$row['name']."</i><br />"
            . "\n<p>".$row['comment']."</p>"
			. "</div>";
			$count++;			
	}
...
if(($this->is_user(@$_COOKIE['normal_user'], @$_COOKIE['normal_pass']) == TRUE) || ($this->is_admin(@$_COOKIE['admin_user'], @$_COOKIE['admin_pass']) == TRUE)) {
    ...
    print "\n<div id=\"commento\">"
        . "\n<form name=\"addcomment\" action=\"?page=view_article_blog&id=".$this->id."&action=send_comment\" method=\"POST\">"
    ...
    if(@$_GET['action'] == 'send_comment') {//aggiunta reale del commento
        ...
        $this->sql->sendQuery("INSERT INTO ".__PREFIX__."comments_blog (blog_id, name, comment, ip, data) VALUES ('".$this->id."', '{$name}', '{$commento}', '{$ip}', '{$data}')");
    
        header('Location: index.php?page=view_article_blog&id='.$this->id);
    }//end if send commit
