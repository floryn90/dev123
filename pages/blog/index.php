...
$result = $this->sql->sendQuery("SELECT COUNT(*) FROM ".__PREFIX__."blog"); 
...
$this->post = $this->sql->sendQuery("SELECT * FROM ".__PREFIX__."blog ORDER BY id DESC LIMIT ".$offset.", ".$limit);
...
while($this->article = mysql_fetch_array($this->post)) { 		
   	print    "\n\t<div id=\"article_top\">"
   			...
   			. "\n\t<div id=\"article\">"
   			. "<br /><br /><br />"
			. "\n<br /><p>".BBCode($this->article['post'])."</p> <br />"
    		. "\n\t</div>";
}
...
if($pager->numPages > 0) {
    print "\n<p align=\"center\">";
        for ($i = 1; $i <= $pager->numPages; $i++) {
            if ($i < $pager->numPages)
                print " <a href=\"index.php?page=blog&pages=".$i."\">[".$i."]</a> -";
            else
                print " <a href=\"index.php?page=blog&pages=".$i."\">[".$i."]</a>";
        }
    print "\n</p>";
